...
def encoder(self, event): 
    ...
    if (event.data1 == nihia.buttons.button_list.get("ENCODER_BUTTON")):
        ...
        if ui.getFocused(config.winName["Mixer"]) == True or ui.getFocused(config.winName["Channel Rack"]) == True or ui.getFocused(config.winName["Plugin"]) == True or ui.getFocused(config.winName["Piano Roll"]) == True:
