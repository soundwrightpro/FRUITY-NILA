...
def adjust_channel_value(channel_value, data2, increment, speed=0):
    ...
    if data2 in range(65, 95):
        return round(channel_value - increment * 2.5, 2)
        ...
        for control_type in (0, 1):  # 0 for volume, 1 for pan
            for z in range(8):
                if channels.channelCount() > z and channels.selectedChannel() < (channels.channelCount() - z):
                    ...
                    knob_speed = self.knob_speed if core.seriesCheck() and event.data2 in (nihia.mixer.KNOB_DECREASE_MAX_SPEED, nihia.mixer.KNOB_INCREASE_MAX_SPEED) else 0
                    ...
                    if event.data1 == knob_data:
                        ...
                        adjusted_increment = config.increment * 2 if time_difference < 0.8 else config.increment
