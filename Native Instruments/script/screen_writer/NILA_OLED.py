...
def get_correct_tracks():
        """Return the tracks displayed on the OLED.

        Returns:
                A list of up to ``c.max_knobs`` track indices.
        """
...
def format_param_name(param_name):
        """Insert spaces into parameter names for OLED display.
...
def OnUpdateBeatIndicator(self, Value):
        """Update the beat indicator on the OLED display.
...
def OnIdle(self):
        """Perform OLED updates while idle.
...
def purge_tracks(start, end, clear_info=False):
        """Clear track slots within a specified range.

        Args:
                start: Starting track index.
                end: Ending track index.
                clear_info: When ``True`` also clear track data.
